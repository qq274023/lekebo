 ...
 * 项目类型：微信小程序
 * 项目名称：蒙牛营养生活家
 * 项目抓包：抓member-api.mengniu.cn下的X-Token & unionId填入变量
 * 项目变量：lekebo_mnflag_Cookie
 * 项目定时：每40分钟运行一次
 * cron: 0 0,7 * * *
...
let UserCookie = ($.isNode() ? process.env.lekebo_mnflag_Cookie : $.getdata('lekebo_mnflag_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
var timestamp = Math.round(new Date().getTime()).toString();
let host = 'member-api.mengniu.cn';
...
function getMemberInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/user/mnme/member/centerInfo`,
            headers: {
                Host: host,
                ...
                'Referer': 'https://servicewechat.com/wx1342c59a70c7a94f/224/page-frame.html',
                'Content-Length': '12',
            },
            body: `{"mer_id":1}`,
        }
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 200) {
                    DoubleLog(`\n 会员信息: ✅ ，${result.data.memberInfo.memberLevelName}:${result.data.memberInfo.mobile} 蛋白质：${parseInt(result.data.memberInfo.proteinBalance)}`)
                } else {
                    DoubleLog(`\n 会员信息: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
 ...
 * @returns {Promise<unknown>}
 */
function signInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/user/mnme/member/signInfo?businessCode=01&unionId=${ck[1]}`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
                'X-Token': 'Bearer ' + ck[0],
                'Content-Type': 'application/json',
                'Referer': 'https://member.mengniu.cn/pages/activity/protein_earn/index?wxUrlType=5&appToken=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJtdWwtY21sLm1lbmduaXUuY24iLCJhdWQiOiJtdWwtY21sLm1lbmduaXUuY24iLCJpYXQiOjE2ODE1MDAyOTEsIm5iZiI6MTY4MTUwMDI5MSwiZXhwIjoxNjg0MDkyMjkxLCJqdGkiOlsxNzAwODc2NCwidXNlciIsInByb2R1Y3Rpb24iLCJ3eDEzNDJjNTlhNzBjN2E5NGYiLCJyb3V0aW5lIl19.Wp92jQ5G-asnrXNSRuAao2uHxUTT_0NhJXbOi5Muy8M&terminal=yundian',
            },
        }
        $.get(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 200) {
                    getNowTime1 = $.time('yyyy-MM-dd');
                    signdays = result.data.rewards
                    for(i = 0; i < signdays.length; i++){
                        datetime = signdays[i].date
                        if (datetime == getNowTime1) {
                            if (signdays[i].isSign == true) {
                                DoubleLog(`\n 签到失败: ❌ ，今天已经签到到了`);
                            } else {
                                await signin(2 * 1000);
                            }
...
function signin(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/user/mnme/member/taskSubmit`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
            ...
            body: `{"businessCode":"01","taskId":113,"unionId":"${ck[1]}"}`,
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
...
function taskList(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/user/mnme/member/taskList?businessCode=01&position=2&unionId=${ck[1]}`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
            ...
            try {
                ...
                if (result.code == 200) {
                    ...
                    for(i = 0; i < lists.length; i++){
                        DoubleLog(`\n 查询任务: ✅ ，${lists[i].businessCode,lists[i].name}`)                  
 ...
 * @returns {Promise<unknown>}
 */
function taskSubmit(businessCode,taskId,timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/user/mnme/member/taskSubmit`,
            headers: {
            ...
            body: `{"businessCode":"${businessCode}","taskId":"${taskId}","unionId":"${ck[1]}"}`,
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
