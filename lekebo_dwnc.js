...
class UserInfo {
    ...
    async tree_info() {
        try {
            ...
            if (result.code == 200) {
                console.log(`\n 查询奖励: ✅ ，账号[${this.index}] 奖品为 ${result.data.name}`);
            } else {
                console.log(`\n 查询奖励: ❌ ，账号[${this.index}]：${result.msg}`);
            }
        } catch (error) {
            console.log(error);
        }
    ...
    async task_watering() { // 浇水 一次40g /80g
        ...
        try {
            ...
            let result = await httpRequest(options);
            if (result.code == 200) {
                rewardNum = result.data.wateringReward.rewardNum
                if (rewardNum >= 0) {
                    DoubleLog(`\n 浇水奖励: ✅ ，账号[${this.index}]  获得 ${rewardNum}g`);
                } else {
                    DoubleLog(`\n 浇水奖励: ✅ ，账号[${this.index}]  ${result.msg}`);
                }
            } else if (result.code == 711110001) {
                DoubleLog(`账号[${this.index}]  浇水[${result.msg}]`);
            } else {
                DoubleLog(`\n 浇水失败: ❌ ，账号[${this.index}]：${result.msg}`);
            }
        } catch (error) {
            console.log(error);
        }
    ...
    async task_watering_reward() { // 累计浇水奖励
        try {
            ...
            let result = await httpRequest(options);
            if (result.code == 200) {
                rewardNum = result.data.rewardNum
                if (rewardNum >= 0) {
                    DoubleLog(`\n 浇水奖励: ✅ ，账号[${this.index}]  获得 ${rewardNum}g`);
                } else {
                    DoubleLog(`\n 浇水奖励: ✅ ，账号[${this.index}]  ${result.msg}`);
    ...
    async task_extra(condition) { // 领取水滴任务累计奖励
        try {
            ...
            //console.log(result);
            if (result.code == 200) {
                DoubleLog(`\n 累计奖励: ✅ ，账号[${this.index}] 获得:${result.msg}[${result.data.num}g]`);
            } else if (result.code == 711020001) {
                DoubleLog(`\n 累计奖励: ❌ ，账号[${this.index}]：${result.msg}`);
            } else {
                DoubleLog(`\n 累计奖励: ❌ ，账号[${this.index}]：${result.msg}`);
            }
        } catch (error) {
    ...
    async get_tree() { // 农场水滴剩余 和信息
        try {
            ...
            if (result.code == 200) {
                this.ckStatus = true
                DoubleLog(`\n 农场进度: ✅ ，账号[${result.data.treeId}] 成熟进度[${result.data.userWateringDroplet}/${result.data.currentLevelNeedWateringDroplet}g]`);
            } else {
                DoubleLog(`\n 农场进度: ❌ ，账号[${this.index}]：${result.msg}`);
            }
        } catch (error) {
            console.log(error);
        }
