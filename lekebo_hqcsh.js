 ...
 * 项目类型：微信小程序
 * 项目名称：好奇车生活
 * 项目抓包：抓channel.cheryfs.cn下的openId & accountId填入变量
 * 项目变量：lekebo_hqcsh_Cookie
 * 项目定时：每天运行两次
 * cron: 0 9,18 * * *
...
let UserCookie = ($.isNode() ? process.env.lekebo_hqcsh_Cookie : $.getdata('lekebo_hqcsh_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
var timestamp = Math.round(new Date().getTime()).toString();
let host = 'channel.cheryfs.cn';
let hostname = 'https://' + host;
let q1 = '647894196522340352'  // 188积分 1.08元
let q2 = '622187839353806848'  // 288积分 1.88元
let q3 = '622187928306601984'  // 588积分 3.88元
let q4 = '622188100122075136'  // 888积分 5.88元
let qiang = q1 //指定抢哪个红包
//=======================================//
!(async () => {
...
function getMemberInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/archer/activity-api/cherycar/getCarUser?openId=${ck[0]}`,
            headers: {
                Host: host,
        ...
        $.get(url, async (error, response, data) => {
            try {
                ...
                if (result.success == true) {
                    DoubleLog(`\n 会员信息: ✅ ，用户：${result.result.phone}`)
                } else {
                    DoubleLog(`\n 会员信息: ❌ ，原因是：${result.success}`)
                }
            } catch (e) {
                DoubleLog(`\n 会员异常: ❌ ，${response}`)
            } finally {
...
function getMemberScore(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/archer/activity-api/common/accountPointLeft?pointId=620415610219683840`,
            headers: {
                Host: host,
                'Content-Type': 'application/json',
        ...
        $.get(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.success == true) {
                    DoubleLog(`\n 查询积分: ✅ ，${result.result} 积分`)
                } else {
                    DoubleLog(`\n 查询积分: ❌ ，原因是：${result.success}`)
                }
            } catch (e) {
                DoubleLog(`\n 积分异常: ❌ ，${response}`)
            } finally {
...
function signin(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/archer/activity-api/signinact/signin`,
            headers: {
                Host: host,
        ...
        $.get(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 200) {
                    if (result.result.message == "今日已签到") {
                        DoubleLog(`\n 签到失败: ❌ ，原因是：${result.result.message}`)
                    } else {
                        DoubleLog(`\n 签到成功: ✅ ，获得 ${result.amount} 积分`)
                    }
                } else {
                    DoubleLog(`\n 签到失败: ❌ ，原因是：${result.result.message}`)
                }
...
function withdraw(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/archer/activity-api/pointsmall/exchangeCard?pointsMallCardId=${qiang}&exchangeCount=1&mallOrderInputVoStr=%7B%22person%22:%22%22,%22phone%22:%22%22,%22province%22:%22%22,%22city%22:%22%22,%22area%22:%22%22,%22address%22:%22%22,%22remark%22:%22%22%7D&channel=1&exchangeType=0&exchangeNeedPoints=188&exchangeNeedMoney=0&cardGoodsItemIds=`,
            headers: {
                Host: host,
                'accept': 'application/json',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF XWEB/6763',//
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 200) {
                    console.log(result)
                    //DoubleLog(`\n 疯抢红包: ✅ ，${result.body.userInfo.nick} 余额：${result.body.goldCoinInfo.virtualCash}，今天收益：${result.body.goldCoinInfo.amountToday} 金币`)
                } else {
                    DoubleLog(`\n 疯抢红包: ❌ ，原因是：${result.message}`)
                }
            } catch (e) {
                DoubleLog(`\n 红包异常: ❌ ，${response}`)
            } finally {
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_hqcsh_Cookie`)
        return;
    }
    return true;
}
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_hqcsh.js`,
        }
