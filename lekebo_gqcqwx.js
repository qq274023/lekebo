 ...
 * 项目类型：微信小程序
 * 项目名称：广汽传祺
 * 项目抓包：抓mall.gacmotor.com下的token填入变量
 * 项目变量：lekebo_gqcqwx_Cookie
 * 项目定时：每40分钟运行一次
 * cron: 0 0,7 * * *
...
let UserCookie = ($.isNode() ? process.env.lekebo_gqcqwx_Cookie : $.getdata('lekebo_gqcqwx_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
    ...
    return new Promise((resolve) => {
        let url = {
            ...
            headers: {
                ...
                'token': ck[0],
                'User-Agent': getUA(),
                'Referer': 'https://servicewechat.com/wx86a1eb5a53a6973b/244/page-frame.html',
                'Content-Length': '2',
            },
            body: `{}`,
        }
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 0000) {
                    if (task == 1) {
                        DoubleLog(`\n 用户信息: ✅ ，当前拥有 ${result.data.gDou} 积分`)
				    } else if (task == 2) {
				    	DoubleLog(`\n 积分查询: ✅ ，任务后拥有 ${result.data.gDou} 积分`);
                    } else {
                        DoubleLog(`\n 查询失败: ❌ ，原因是：${data}`)
                    }
                } else {
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 0000) {
                    if (result.data.gdouNum == 0) {
                        DoubleLog(`\n 登录签到: ✅ ，原因是：今天已经领取过奖励`)
                    } else {
                        DoubleLog(`\n 登录签到: ✅ ，获得 ${result.data.gdouNum} 积分`)
                    }
                } else {
                    DoubleLog(`\n 登录失败: ❌ ，原因是：${result.msg}`)
                }
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_gqcqwx.js`,
        }
