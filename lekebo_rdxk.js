...
async function start() {
    ...
    await sign(2 * 1000);
    await $.wait(2000);
    await signtoday(2 * 1000);
    await $.wait(2000);
    await setSpread(2 * 1000);
    await $.wait(2000);
 ...
 * @returns {Promise<unknown>}
 */
function signtoday(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/api/usersign/index`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
				'Authori-zation': 'Bearer ' + ck[0],
        ...
        $.get(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.status == 200) {
                    DoubleLog(`\n 连续签到: ✅ ，已连续签到 ${result.data.is_today_sign} 天`)
                } else {
                    DoubleLog(`\n 连续签到: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
 ...
 * @returns {Promise<unknown>}
 */
function setSpread(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/api/user/setSpread`,
            headers: {
                'Host': host,
				'Content-Type': 'application/json',
                'Form-type': 'app',
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_rdxk.js`,
        }
