 ...
 * 项目类型：微信小程序
 * 项目名称：百草味官方商城
 * 项目抓包：抓api8.vipbcw.com下的token & unionId填入变量
...
let UserCookie = ($.isNode() ? process.env.lekebo_bcw_Cookie : $.getdata('lekebo_bcw_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
var timestamp = Math.round(new Date().getTime()/1000).toString();
let host = 'api8.vipbcw.com';
...
async function start() {
    ...
    await signin(2 * 1000);
    await $.wait(2000);
    await pointsign(2 * 1000);
    await $.wait(2000);
    await finishTask(2 * 1000);
    await $.wait(2000);
    await finishTask1(2 * 1000);
    await $.wait(2000);
...
function getMemberInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n3/user/getUserInfo`,
            headers: {
                Host: host,
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': '41354fec2638c805ab16047a2a8e59e6',
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 200) {
                    DoubleLog(`\n 会员信息: ✅ ，用户：${result.data.mobile} 拥有：${result.data.availPoint} 金币`)
                } else {
                    DoubleLog(`\n 会员信息: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
...
function signin(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNxSign/sign`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': '4a5ee4aa574f7838b79321769a1dcf7c',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
                'Content-Length': '2',
            },
            body: '{}',
        ...
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 200) {
                    DoubleLog(`\n 签到成功: ✅ ，${result.data.vipDayPoint} 金币`)
                } else {
                    DoubleLog(`\n 签到失败: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
 ...
 * @returns {Promise<unknown>}
 */
function pointsign(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/doublePoint/index`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': '4076da722f65f54c927384ba65df0df2',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
                'Content-Length': '2',
            },
            body: '{}',
 ...
 * @returns {Promise<unknown>}
 */
function finishTask(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNxSign/finishTask?taskId=1`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': 'd7adee8ee8aaed4fe6b1c9d4859f4c24',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
                'Content-Length': '2',
            },
        }
 ...
 * @returns {Promise<unknown>}
 */
function finishTask1(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNxSign/finishTask?taskId=2`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
 ...
 * @returns {Promise<unknown>}
 */
function finishTask2(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNxSign/finishTask?taskId=12`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': 'b65ab6c64bd5b96009b9ec8fff7d573c',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
                'Content-Length': '2',
            },
        }
 ...
 * @returns {Promise<unknown>}
 */
function goldInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNewPoint/goldInfo`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': '5e33229739a9af7e31a722bacbd53e54',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
                'Content-Length': '53',
            },
            body: `{"fromId":0,"unionId":"${ck[1]}"}`,
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
                if (result.code == 200) {
                    lists = result.data.marketElevenConfigList
 ...
 * @returns {Promise<unknown>}
 */
function goldPoint(tasklist,timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/n6/marketNewPoint/browse?id=${tasklist}&source=3`,
            headers: {
                Host: host,
				'Content-Type': 'application/json',
                'User-Agent': getUA(),
                ...
                'Accept': '*/*',
				'token': ck[0],
                'sign': '692c48d5f9c567100a60b3e80eca9edd',
                'Referer': 'https://servicewechat.com/wx9dddd0217ebb9d71/315/page-frame.html',
            },
        }
        $.get(url, async (error, response, data) => {
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_bcw_Cookie`)
        return;
    }
    return true;
}
...
function delay() {
    ...
    if (time > 3000) {// 大于30s重新生成
        return delay();
    } else {
        console.log('\n 随机延时: ', `${time}ms, 避免大家运行时间一样`)
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/qq274023/lekebo/master/lekebo_bcw.js`,
        }
